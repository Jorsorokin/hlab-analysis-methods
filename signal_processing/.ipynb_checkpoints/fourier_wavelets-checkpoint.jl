...
# what happens if we remove some frequencies that we don't care about? Can we "reconstruct" the resulting time-series?
F = DSP.fft(x)
F[100:end] = 0
x_filt = DSP.ifft(F)
plot(x, label="original")
plot!(x_filt, label="first 100 fourier components")
